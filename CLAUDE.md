...
**Phase 2 hardcoded content refactor (2026-04-08):** Extracted all hardcoded content from components. Created `content/site.json` (address, phone, email, social links) — loaded in `layout.tsx`, passed to `SiteFooter`. Added `topLevelLinks` to `content/nav.json` — `SiteHeader` passes to `MobileMenu` as prop. Deleted `src/data/nav.ts`; `SiteHeader` now uses `getNav()` from `@/lib/content`. Replaced `WeddingGallery` (hardcoded images) with `VenueGallery` (fetches `/api/gallery`, filters to "Venue Hire", reuses `GalleryGrid`/`GalleryModal`). Added `galleryHeading` to `content/pages/venue.json`.
